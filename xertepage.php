	...
	if($post->post_type=="xerte_online"){
		...
		if($post->post_status!="publish"){
			...
			if($post->post_status=="draft"){
				...
				if($current_user->allcaps['edit_posts']==1&&$current_user->allcaps['edit_others_posts']==1&&$current_user->allcaps['edit_published_posts']==1){
				...
				wp_die("This post is not yet published");
		...
		if($show){
			...
			$wpdb->query("insert into " . $table_name . "(post_id,user_id,date_accessed)VALUES('" . $post->ID . "','" . $user_ID . "','" . time() . "')");
		
			if(isset($_GET['preview'])){
		
				if($_GET['preview']=="true"){
			
					$string_for_flash_xml = "wp-content/uploads/xerte-online/" . $post->ID . "/preview.xml?time=" . time();
			...
			<script type="text/javascript" src = "<?PHP echo site_url(); ?>/wp-content/plugins/xerte-online/xertefiles/rloObject.js"></script>
